...
import {
  ...
  listPatientOverrides,
  upsertPatientOverride,
} from '@/lib/invoicing/priceLists';
import { TreatmentCodeNotFoundError } from '@/lib/invoicing/treatments';
import { patientPriceOverrideUpsertSchema } from '@/lib/invoicing/validation';
...
export async function GET(ctx: { params: Promise<{ patientId: string }> }) {
  ...
  try {
    const overrides = await listPatientOverrides(tenant.prisma, patientId);
    ...
    console.error('Failed to list patient overrides', error);
...
export async function POST(req: Request, ctx: { params: Promise<{ patientId: string }> }) {
  ...
  const parsed = patientPriceOverrideUpsertSchema.safeParse(data);
  if (!parsed.success) {
  ...
  try {
    const override = await upsertPatientOverride(tenant.prisma, patientId, parsed.data);
    return NextResponse.json({ ok: true, item: override });
  } catch (error) {
    ...
    console.error('Failed to upsert patient override', error);
    return NextResponse.json({ ok: false, message: 'Server error' }, { status: 500 });
  }
}
